...
func (b *Bridge) BuildAggregateTransaction(relayFeePerKb int64, addrs []string, utxos []*electrs.ElectUtxo) (rawTx *txauthor.AuthoredTx, err error) {
	...
	script, version, err := b.GetPayToAddrScript(cfgUtxoAggregateToAddress)
	if err != nil {
		return nil, err
	}
...
func (b *Bridge) getUtxosFromElectUtxos(target dcrnAmountType, addrs []string, utxos []*electrs.ElectUtxo) (detail *txauthor.InputDetail, err error) {
	...
	for i, utxo := range utxos {
		...
		address := addrs[i]
		pkScript, _, errt := b.GetPayToAddrScript(address)
		if errt != nil {
			continue
		}
